...
resource "azurerm_key_vault_key" "encryption_scope2" {
  name         = "tfex-key-1"
  key_vault_id = azurerm_key_vault.encryption_scope.id
  key_type     = "RSA"
  key_size     = 4096 # 2048, 3072 and 4096.
  key_opts = [
    "decrypt",
    "encrypt",
    "sign",
...
resource "azurerm_storage_encryption_scope" "encryption_scope2" {
  name               = "akvmanaged1"
  storage_account_id = azurerm_storage_account.encryption_scope.id
  source             = "Microsoft.KeyVault"
  key_vault_key_id   = azurerm_key_vault_key.encryption_scope2.id
  infrastructure_encryption_required = true
}


resource "azurerm_storage_container" "encryption_scope2" {
  name                  = "vhds2"
  storage_account_id    = azurerm_storage_account.encryption_scope.id
